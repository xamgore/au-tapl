...
match (PRcd []) _   = Just []
match (PRcd ((sym, pat):ps)) (Rcd vs) = do
  val  <- snd <$> find ((sym ==) . fst) vs
  set  <- match pat val
  rest <- match (PRcd ps) (Rcd vs)
  return $ set ++ rest
match _ _ = Nothing
...
rec = Rcd  [("lz",Tru),("ly",Fls),("lx",Fls)]

e1 = match pat rec
t1 = e1 == Just [("px",Fls),("py",Fls)]

e2 = oneStep $ Let pat rec $ If Tru (Idx 1) (Idx 0)
t2 = e2 == Just (If Tru Fls Fls)
